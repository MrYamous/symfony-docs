...
When the ``widget`` option is set to ``choice`` or ``text``, each sub-field
(year, month, and day) renders a label. By default, these labels are
auto-generated by capitalizing the field name (``Year``, ``Month``, ``Day``).

Use an array to customize the label for each field::
